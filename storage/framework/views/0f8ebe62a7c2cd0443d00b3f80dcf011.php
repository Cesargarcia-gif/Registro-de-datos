<div id="tabla-wrapper">
        ...
        <div id="tabla-scroll" class="max-h-[867px] overflow-y-auto border border-gray-700 rounded-lg">
            ...
            <table id="notasTable" class="w-full text-sm text-left text-white border-collapse">
                ...
                <tbody class="bg-gray-800 text-white">
                    <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php $__currentLoopData = $notas; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $nt): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                    <tr class="border-b border-gray-300 hover:bg-gray-700 cursor-pointer
                        <?php echo e($selectedNoteId === $nt->id ? 'bg-blue-600' : ''); ?>"
                        wire:click="select(<?php echo e($nt->id); ?>)">

                        <td class="px-2 py-4 border border-gray-300 font-medium"><?php echo e($nt->codigo); ?></td>
                        <td class="px-8 py-4 border border-gray-300 font-medium"><?php echo e($nt->title); ?></td>
                        <td class="px-8 py-4 border border-gray-300"><?php echo e($nt->edad); ?></td>
                        <td class="px-8 py-4 border border-gray-300"><?php echo e($nt->telefono); ?></td>
                        <td class="px-8 py-4 border border-gray-300"><?php echo e($nt->direccion); ?></td>
                        <td class="px-8 py-4 border border-gray-300">Q<?php echo e(number_format($nt->abono, 2)); ?></td>
                        <td class="px-8 py-4 border border-gray-300">Q<?php echo e(number_format($nt->abonose, 2)); ?></td>
                        <td class="px-8 py-4 border border-gray-300">Q<?php echo e(number_format($nt->abono + $nt->abonose, 2)); ?></td>
                        <td class="px-8 py-4 border border-gray-300"><?php echo e($nt->turno); ?></td>
                        <td class="px-8 py-4 border border-gray-300"><?php echo e($nt->description); ?></td>

                        <td class="px-8 py-4 border border-gray-300 text-center">
                            <button class="print-btn text-blue-400 hover:text-blue-200 text-xl"
                                data-url="<?php echo e(route('note.imprimir', $nt->id)); ?>" title="Imprimir">
                                🖨️
                            </button>
                        </td>
    ...
    <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($selectedNoteId): ?>
        <div class="cinta-negra"></div>

        <div class="fixed bottom-2 right-2 z-50 flex gap-2 justify-end">
            <a href="<?php echo e(route('note.edit', $selectedNoteId)); ?>"
                class="px-3 py-1 bg-blue-600 text-white text-xs rounded hover:bg-blue-700">
                Editar
            </a>

            <button onclick="confirmDelete(event, <?php echo e($selectedNoteId); ?>)"
...
function confirmDelete(event, noteId) {
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            window.Livewire.find('<?php echo e($_instance->getId()); ?>').call('delete', noteId);
        }
    });
}
...
document.addEventListener('click', function(event) {
    ...
    if (!tabla.contains(event.target)) {
        window.Livewire.find('<?php echo e($_instance->getId()); ?>').set('selectedNoteId', null);
    }
});

</script>
<?php /**PATH C:\Users\Asistente IT 2\Documents\crudLivewire prueba login\resources\views/livewire/test.blade.php ENDPATH**/ ?>